...
// @route   POST /api/staff/me/documents
// @access  Staff
...
//     const fileUrl = `/uploads/staff-documents/${req.file.filename}`;
...
//     });
//   } catch (err) {
//     console.error("UPLOAD MY DOCUMENT ERROR:", err);
...
// @route   DELETE /api/staff/me/documents/:documentId
// @access  Staff
// @route   DELETE /api/staff/me/documents/:documentId
// @access  Staff
export const deleteMyDocument = async (req, res) => {
  try {
    ...
    // ---------------------------------------------
    // Save S3 key before deleting Mongo document
    // ---------------------------------------------
    ...
    if (fileKey) {
      try {
        ...
        // MongoDB deletion already happened.
        // We don't want to return a failed
        // response just because S3 deletion failed.
      }
    }
...
export const deleteStaff = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      staff: { _id: staff._id, fullName: staff.fullName },
    });
  } catch (err) {
    return res.status(500).json({ message: "Server error", error: err.message });
  }
