...
// ─── Shared field list ────────────────────────────────────────────────────
// Single source of truth for every rating field on a Psy mark. Add a new
// trait here once and savePsy / updateMark / updateMarks / getScores /
// getPsybyStudent all pick it up automatically.
const RATING_FIELDS = [
  // Affective Traits
  "attentiveness",
  "attitudeToWork",
  ...
  "processingSpeed",
  "retentiveness",
  // Legacy fields (older exams saved before the field expansion)
  "instruction",
  "independently",
  "talking",
  "eyecontact",
...
const TEXT_FIELDS = ["remarks", "premarks"];

// Picks only the known fields off an incoming update object, defaulting
// missing ratings to 0 and missing text fields to "".
const pickMarkFields = (src = {}) => {
  const out = {};
...
// Builds the { "marks.$[elem].field": value } object for $set updates.
const buildSetOps = (src = {}) => {
  const ops = {};
  RATING_FIELDS.forEach((f) => {
    ops[`marks.$[elem].${f}`] = src[f] ?? 0;
  });
  TEXT_FIELDS.forEach((f) => {
    ops[`marks.$[elem].${f}`] = src[f] ?? "";
  });
  return ops;
};
...
export const savePsy = async (req, res) => {
  try {
    ...
    existingMarks.marks.forEach((existingMark) => {
      ...
      if (update) {
        const fields = pickMarkFields(update);
        Object.assign(existingMark, fields);
      }
    });
...
export const getPsybyStudent = async (req, res) => {
  try {
    ...
    const filteredMarks = psyRecords.flatMap((record) =>
      record.marks
        .filter((mark) => {
          if (mark.studentId.toString() !== studentId) return false;
          if (record.examId.name !== examName) return false;

          // Ensure at least one rating field is non-zero, or a remark was left
          const hasRating = RATING_FIELDS.some((f) => (mark[f] ?? 0) !== 0);
          const hasRemark =
            (mark.remarks && mark.remarks.trim() !== "") ||
            (mark.premarks && mark.premarks.trim() !== "");

          return hasRating || hasRemark;
        })
        .map((mark) => ({
          examId: record.examId._id,
          examName: record.examId.name,
          ...pickMarkFields(mark.toObject()),
        }))
    );
  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (!updatedDoc) {
        ...
        const newMark = {
          studentId,
          ...pickMarkFields(update),
        };

        const pushFilter = { examId };
        const pushUpdate = { $push: { marks: newMark } };
        const pushOptions = { upsert: true, new: true };

        updatedDoc = await Psy.findOneAndUpdate(
          pushFilter,
          pushUpdate,
          pushOptions
        );
